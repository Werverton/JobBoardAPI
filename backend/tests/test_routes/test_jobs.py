...
def test_update_a_job(client):
    ...
    response = client.get("/jobs/get/1/") #recuperando pelo id
    
    assert response.status_code == 200 #fazendo assert do status code

    assert response.json()['title'] == "test new title" #verificando se o title foi alterado como ali em cima
